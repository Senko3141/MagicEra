	...
	for i = 1, data.points do
		...
		if data.movement then
			...
			coroutine.wrap(function()
				while moving do
					local rayCheck = _G.CastRay(Part.Position+Vector3.new(0,data.size,0),Vector3.new(0,-10,0),data.blacklist)
					if rayCheck then
		...
		coroutine.wrap(function()
			...
			if not data.domino then
				tween = tweenService:Create(Part,TweenInfo.new(1,Enum.EasingStyle.Quad,Enum.EasingDirection.InOut),{Size = Vector3.new(0,0,0),Orientation = Vector3.new(math.random(-90,90),math.random(-90,90),math.random(-90,90))})
				tween:Play()
	...
	coroutine.wrap(function()
		if data.domino then
			for i,v in ipairs(model:GetChildren()) do
				tweenService:Create(v,TweenInfo.new(data.clearSpeed,Enum.EasingStyle.Quad,Enum.EasingDirection.InOut),{Size = Vector3.new(0,0,0),Orientation = Vector3.new(math.random(-90,90),math.random(-90,90),math.random(-90,90))}):Play()
