...
Remotes.LootRemote.OnServerEvent:connect(function(player, args, target)
	if args == "Start" then
		print("created effect")

		local attachment0 = Instance.new("Attachment")
		attachment0.Name = "LootAttachment"
		attachment0.Parent = player.Character["Right Arm"]


		local effect = ReplicatedStorage.Assets.LootBeam:clone()
		effect.Parent = player.Character["Right Arm"]
		effect.Attachment0 = attachment0
		effect.Attachment1 = workspace.Effects:FindFirstChild("Orb"..target).Attachment
		print(effect)

	elseif args == "End" then
		for i,v in pairs(player.Character:GetDescendants()) do
			if v.Name == "LootAttachment" then
				v:Destroy()
			elseif v.Name == "LootBeam" then
				v:Destroy()
			end
		end
	elseif args == "Loot" then
		local data = player.Data
		for i,v in pairs(workspace.LootStore:GetDescendants()) do
			if v.Parent and v.Parent.Name == target then
				local itemInfo = ShopItems[v.Name] or nil
				if itemInfo ~= nil then
					for a,b in pairs(data:GetDescendants()) do
						if b.Name == v.Name then
							b.Value += v.Value
							print(b)
							v:Destroy()
							Remotes.Notify:FireClient(player, "Successfully looted "..b.Name.." !", 4, {
								Sound = "Notification"
							})
						end
					end
				...
				if data.Items:FindFirstChild(itemInfo.Category):FindFirstChild(v.Name) == nil then
					local newValue = Instance.new("IntValue")
					newValue.Name = v.Name
					newValue.Value += v.Value
					newValue.Parent = data.Items:FindFirstChild(itemInfo.Category)
					Remotes.Notify:FireClient(player, "Successfully looted "..v.Name.." !", 4, {
						Sound = "Notification"
					})
					v:Destroy()
				end
		...
		for i,v in pairs(workspace:GetDescendants()) do
			if v:IsA("Part") and v.Name == target then
				v:Destroy()
			elseif v:IsA("Folder") and v.Name == target then
				v:Destroy()
			end
		end
		for i,v in pairs(workspace.Effects:GetChildren()) do
			if v.Name == "Orb"..target then
				v:Destroy()
			end
		end
...
--//Remove chests if player leaves :3
...
Remotes.Clothing.OnServerEvent:connect(function(player, action, num)
	...
	if action == "Start" then
		local name, statTable, empty, statToString = clothingMod.ReturnInformation(player, numberOn)
		local stat
		local amt
		local toVariable = function()
			for i,v in pairs(statTable) do
				stat = i
				amt = v
				Remotes.Clothing:FireClient(player, statTable, name, numberOn, statToString)
			end
		end
		toVariable()
	...
	elseif action == "Forward" then
		local min,max = clothingMod.ReturnMinMax()
		numberOn += 1
		if numberOn >= max then
			numberOn = max
		end
		local name, statTable, empty, statToString = clothingMod.ReturnInformation(player, numberOn)
		local stat
...
Remotes.Footsteps.OnServerEvent:Connect(function(player, sound, root)
	Remotes.Footsteps:FireAllClients(sound,root)	
end)
...
Remotes.Swim.OnServerEvent:Connect(function(plr, movement)
	local char = plr.Character

	if movement == "Swim" then
		local params = RaycastParams.new()
		params.FilterType = Enum.RaycastFilterType.Whitelist
		params.FilterDescendantsInstances = {workspace.Water}
		local raycast = workspace:Raycast(char["HumanoidRootPart"].Position, char["HumanoidRootPart"].CFrame.UpVector * -50, params)
		if raycast then
			local splash = Instance.new("Vector3Value")
			splash.Name = "Splash"
			splash.Value = raycast.Position
...
Remotes.TeleportTo.OnServerEvent:connect(function(player)
	local suc,ero = pcall(function()
		TS:Teleport(11817538912, player, {SlotToLoad = player:FindFirstChild("Slot").Value})
		print("teleporting")
	end)
end)